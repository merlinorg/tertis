  ...
  def render(
    ...
    def drawPoly(tint: Color, vertices: Float*): Unit =
      ...
      val polygonRegion = new PolygonRegion(
        ...
        vertices.toArray,
        if vertices.length == 8 then // counterclockwise triangles
    ...
    if !test(0, -1) then // nothing below
      val l = if test(-1, 0) then 0 else v
      val r = if test(1, 0) then 0 else v
      drawPoly(shadow, 0, 0, w, 0, w - r * 2, v * 2, l * 2, v * 2)
      drawPoly(shadow, 0, 0, w, 0, w - r, v, l, v)
    else if test(-1, 0) && !test(-1, -1) then
    ...
    if !test(1, 0) then // nothing to the right
      val b = if test(0, -1) then 0 else v
      val t = if test(0, 1) then 0 else v
      drawPoly(shadow, w - v * 2, b * 2, w, 0, w, h, w - v * 2, h - t * 2)
      drawPoly(shadow, w - v, b, w, 0, w, h, w - v, h - t)
    else if test(0, 1) && !test(1, 1) then
      drawPoly(shadow, w - v * 2, h - v * 2, w, h, w - v * 2, h)
      drawPoly(shadow, w - v, h - v, w, h, w - v, h)

    if !test(0, 1) then // nothing above
      val l = if test(-1, 0) then 0 else v
      val r = if test(1, 0) then 0 else v
    ...
    if !test(-1, 0) then // nothing to the left
      val b = if test(0, -1) then 0 else v
      val t = if test(0, 1) then 0 else v
