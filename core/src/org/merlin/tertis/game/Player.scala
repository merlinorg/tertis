  ...
  def update(delta: Float, oldLoc: BlockLoc): Unit =
    ...
    newLocations.find(isValid(game, _)) match
      case Some(newLoc) =>
        ...
        if newLoc.rotation != oldLoc.rotation then game.rotate = None // wrong
        if newY >= newLoc.y then                                      // moved full amount
          touchdown = None
        else if !shifted && !rotated && touchdown.exists(_ >= GracePeriodSeconds) then
          game.board.drop(
            block,
            newLoc.rotation,
            newLoc.column,
        ...
        else touchdown = touchdown.map(_ + delta).orElse(Some(0f))
        ...
        if touchdown.isEmpty && isValid(game, nextLoc) then game.clickPlayed = false
        else if !game.clickPlayed then
