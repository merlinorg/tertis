...
/** Identifier allows us to identify all the pixels of a single block. They may get split if a row is dropped.
  */
...
//    for (j <- 0 until 20 * Columns) if (j % Columns > 0) board.update(j, reg)
  ...
  def update(delta: Float): Unit =
    ...
    else if animating then
      if redAlpha < 1f then redAlpha = redAlpha.alphaUp(delta, RedFadeSeconds)
      else
        redVelocity = redVelocity + delta / RedShiftAccelerationSeconds
        val xShift = redShift
        redShift = (redShift + redVelocity) min dropRows.size.toFloat
        if redShift >= dropRows.size then
          var drop = 0
          var mass = 0

          for (j <- 0 until BoardRows)
            while dropRows.contains(j + drop) do drop = drop + 1
            for (i <- 0 until Columns)
              val region = get(i, j + drop)
              set(i, j, region)
              if drop > 0 && (region ne null) then mass = mass + 1

          if !Prefs.MuteAudio.isTrue && mass > 0 then
            Tertis.crash.play(
        ...
        else if xShift < firstChunk && redShift >= firstChunk then
          if !Prefs.MuteAudio.isTrue then Tertis.crash.play(15f / 400f) // arbitrary..

  def draw(batch: PolygonSpriteBatch): Unit =
    var dropped = 0
    for (j <- 0 until BoardRows)
      if dropRows.contains(j) then dropped = dropped + 1
  ...
  def drop(
    ...
    for (j <- 0 until BoardRows)
      ...
      if drop then dropRows.add(j)

      if dropping != drop then
        regionMap.clear()
        if dropping && firstSpan then
          // have I just ended the first span of drops
          firstSpan = false
          firstChunk = dropRows.size

      if drop || dropping != drop then
        for (i <- 0 until Columns)
          val region = get(i, j)
          if region ne null then
            ...
            if flash.contains(region.identifier) then flash.add(newRegion.identifier)
        dropping = drop

      if dropRows.nonEmpty then
        for (i <- 0 until Columns)
          val region = get(i, j)
          val mapped =
            if region eq null then region
            else regionMap.getOrElse(region.identifier, region)
          set(i, j, mapped)

  def rowIsFull(row: Int): Boolean =
    (row >= 0) && (row < BoardRows) && (0 until Columns).forall(column => board(row * Columns + column) ne null)
