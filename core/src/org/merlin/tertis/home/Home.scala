...
//    ),
  ...
  override def update(delta: Float): Option[Scene] =
    ...
    if state == HomeState then
      ...
      if logoAlpha > PlayDelaySeconds then playAlpha = playAlpha.alphaUp(delta, PlayFadeInSeconds)
      None
    else
      logoAlpha = logoAlpha.alphaDown(delta, LogoFadeOutSeconds)
      playAlpha = playAlpha.alphaDown(delta, PlayFadeOutSeconds)
      if state == SettingsState then (logoAlpha + playAlpha == 0f).option(new Settings(this))
      else if state == PlayState then (logoAlpha + playAlpha == 0f && Frame.alpha == 1f).option(nextGame)
      else (logoAlpha + playAlpha == 0f).option(new Help(this, (state == HelpPlayState).option(nextGame)))
  ...
  override def render(batch: PolygonSpriteBatch): Unit =
    ...
    if paused.isDefined && !discard then drawPaused(batch)
    else
      for
        score <- Prefs.HighScore.intValue
        time  <- Prefs.HighTime.intValue
      do drawHighScore(batch, score, time)
    Text.draw(
      batch,
      Text.tinyFont,
      Color.DARK_GRAY ⍺ (.25f * playAlpha),
      s"v${Tertis.version}",
      l => (Geometry.ScreenWidth - l.width - Geometry.Dimension / 4) -> (l.height + Geometry.Dimension / 4)
    )
  ...
  private def drawHighScore(
    ...
    if Tertis.globalHigh > 0 then
      val (allScore, allTime) =
        if score > Tertis.globalHigh then (score, time)
  ...
  def play(): Unit =
    if Prefs.Instructed.booleanValue.contains(true) then
      state = PlayState
...
object Home:
  ...
  enum State:
    case HomeState, HelpState, HelpPlayState, SettingsState, PlayState
